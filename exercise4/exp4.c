...
char* memory_info_read();	/*内存使用情况*/
char* stat_read();	/*cpu使用率*/
char* process_sum_read();	/*进程数*/
...
int main( int argc,char *argv[] )
{
    ...
    g_signal_connect(button1, "clicked",G_CALLBACK (kill_proc), clist);
...
{
    gtk_label_set_text(GTK_LABEL(process_label), process_sum_read());
    gtk_widget_show(reinterpret_cast<GtkWidget *>(&process_label));
    return TRUE;
}
...
gint cpu_refresh(gpointer cpu_label)
{
    ...
    gtk_widget_show(reinterpret_cast<GtkWidget *>(&cpu_label));
    return TRUE;
}
...
{
    gtk_label_set_text(GTK_LABEL(mem_label), memory_info_read());
    gtk_widget_show(reinterpret_cast<GtkWidget *>(&mem_label));
    return TRUE;
}
